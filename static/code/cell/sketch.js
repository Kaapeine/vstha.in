...
class Row {
  ...
  drawCell(i) {
    // strokeWeight(1);
    noStroke();
    if (this.state[i] == 0) fill(bgColor);
    else fill(activeCell);

    push();
    translate(this.offset, 0);
    rect(
      i * this.cellWidth,
      this.t * this.cellHeight,
      this.cellWidth,
      this.cellHeight
    );
    stroke(255, 0, 0);
  ...
  updateState(l, c, r) {
    ...
    if (curr.equals(s0)) newState = 0;
    if (curr.equals(s1)) {
      newState = 1;
    }
    if (curr.equals(s2)) newState = 1;
    if (curr.equals(s3)) newState = 1;
    if (curr.equals(s4)) newState = 1;
    if (curr.equals(s5)) newState = 0;
    if (curr.equals(s6)) newState = 0;
    if (curr.equals(s7)) newState = 0;
